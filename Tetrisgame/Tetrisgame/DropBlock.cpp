...
{
	elapsedTime += deltaTime * timeModifier;	// 시간 누적
	if (elapsedTime > dropInterval)	// 누적된 시간이 dropInterval을 넘어섰으면
	{
		MoveDown();					// 한 칸 아래로 이동시키는 함수 실행
...
void DropBlock::Reset()
{
	...
	currentPosition = {
		static_cast<int>((Stage::GetStageWidth() / 2 - current->GetWidth()) / 2),
...
void DropBlock::Spin()
{
	...
	{
		current->ReverseSpin();	// 돌렸을 때 적절하지 않은 위치가 있으면 다시 되돌리기
	}
}
