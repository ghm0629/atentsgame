...
private:
	...
	/// <param name="pTargetPositon">체크할 위치(null이면 블록의 위치를 사용)</param>
	...
	/// <summary>
	/// block이 한 칸 아래로 내려온 상황에서 처리해야 할 일을 하는 함수
	/// </summary>
	/// <param name="block">내려온 블록</param>
	/// <returns>true 성공적으로 내려옴, false면 막혀서 못 내려감</returns>
	bool MoveDownProcess(const DropBlock& block);

	// 하드드랍 처리용 함수
	void HardDropProcess(const DropBlock& block);

	// 월 킥 처리용 함수(리턴이 true면 성공, false면 실패)
	...
	/// <summary>
	/// lineData에 block을 basePosition 위치에 추가하는 함수
	/// </summary>
	/// <param name="basePosition">기준 위치</param>
	/// <param name="block">추가할 블럭</param>
	void AddLine(Position& basePosition, const DropBlock& block);

	/// <summary>
	/// 가득찬 줄을 제거하는 함수
	/// </summary>
	/// <param name="start">체크 시작할 줄(제일 아랫줄)(</param>
	...
	// 게임 오버인지 체크하는 함수(true면 게임오버, false면 게임
	bool CheckGameOver();
