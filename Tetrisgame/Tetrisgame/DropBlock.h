...
public:
	...
	// 키 입력으로 실행시키는 함수들
	void MoveLeft();	// 왼쪽으로 한 칸 움직이는 함수
	void MoveRight();	// 오른쪽으로 한 칸 움직이는 함수
	...
	// 좌우로 움직였음을 알리는 함수 객체(리턴값이 true면 정상적으로 이동할 수 있다. false면 이동 불가능한 지역으로 이동했다)
	std::function<bool(const DropBlock&)> onMoveSide = nullptr;
	std::function<bool(const DropBlock&)> onMoveDown = nullptr;	// 아래로 한 칸 내려갔음을 알리는 함수 객체
