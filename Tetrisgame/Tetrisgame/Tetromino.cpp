...
void Tetromino::Initialize()
{
	...
	CollectPositions(); // data를 기반으로 minoPositions를 채우기
}

void Tetromino::Destroy()
{
	if (data != nullptr)	// data가 nullptr이 아니면
	{
		// data delete하기
		for (int i = 0; i < SpinCount; i++)	// SpinCount만큼 반복
		{
...
void Tetromino::Spin()
{
	...
	spinIndex %= SpinCount;	// spinIndex는 0 ~ 3이 계속 반복된다.
}

void Tetromino::ReverseSpin()
{
	spinIndex = (spinIndex + SpinCount - 1) % SpinCount;			// spinIndex 1 감소 + spinIndex는 0~3의 범위를 가진다.
}

void Tetromino::CollectPositions()
{
	// data의 내용을 기반으로 minoPositioins를 채우기
	...
	int index = 0;						// minoPositions의 특정 방향의 몇 번째 블럭인지 나타내는 변수
	int size = width * height;			// 테트로미노의 크기(파생클래스 별로 다름)
	for (int i = 0; i < SpinCount; i++)	// 방향별로 처리
	{
		index = 0;						// index 초기화
		for (int j = 0; j < size; j++)	// 테트로미노의 크기만큼 반복
		{
			if (*(data[i] + j) == CellType::Block)	// i방향일 때 j번째 칸이 Block인지 확인
